  ...
  const pillars = [
    {
      ...
      whatsappMsg: "Hola, me interesa agendar una Asesoría de evaluación.",
      featured: false,
    },
    {
      step: "02",
      ...
      whatsappMsg: "Hola, quiero cotizar la optimización de mi equipo o flota.",
      featured: true,
    },
    {
      step: "03",
      ...
      whatsappMsg: "Hola, necesito orientación para cambiar un componente.",
      featured: false,
    },
  ];

  return (
    <section id="servicios" className="py-16 sm:py-24 bg-[var(--paper-2)] border-b border-[var(--line)]">
          ...
          {pillars.map((pillar) => (
            ...
            >
              <div className="flex flex-col gap-4">
                ...
                <ul className="flex flex-col gap-2 my-2">
                  {pillar.points.map((pt, i) => (
                    <li key={i} className="font-sans text-xs text-[var(--ink)] flex items-start gap-2">
              ...
              <div className="pt-6 border-t border-[var(--line)] mt-6">
                <a
                  href={getWhatsAppUrl(pillar.whatsappMsg)}
                  target="_blank"
                  rel="noopener noreferrer"
                  className={`w-full font-sans font-bold text-sm py-3 px-4 rounded-full text-center transition-all flex items-center justify-center gap-2 border border-[var(--ink)] ${
                    pillar.featured
                      ? "bg-[var(--blue)] text-white hover:bg-[var(--ink)] hover:text-white shadow-md"
                      : "bg-[var(--ink)] text-[var(--paper)] hover:bg-[var(--ink-soft)]"
                  }`}
                >
                  <span>💬</span> {pillar.ctaText}
                </a>
              </div>
            </div>
          ))}
